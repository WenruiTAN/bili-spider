...
with main_col:
    ...
    if not st.session_state.clicked:
        ...
        with col_desc:
            ...
            * **找得准**：它只找标题里完全符合关键词的视频，那些乱七八糟的干扰项会被自动过滤。
            * **不费劲**：不用去数 B 站有多少页，它会自动埋头苦干，搜到最后一页就自动收工。
            * **好上手**：导出的 Excel 是**“纯数字版”**。下载后点一下表头就能直接排序，不用再手动调格式。
            
            ### 📖 操作指南
            1. **填入 Cookie**：在下方框内粘贴（侧边栏有 10 秒获取教程）。
